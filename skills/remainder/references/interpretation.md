...
The source supports providers `codex`, `claude`, `grok`, `cursor`, and `devin` with profile `default`.
...
Release v0.4.0 adds the Devin CLI provider.
Linux Cursor source fixtures do not certify native access.
...
Devin uses `daily` and `weekly` percentage windows plus `acu`, `prompt_credits`, `flow_credits`, and `flex_credits` windows when the source provides them; its `-1` sentinel means unlimited, and a missing `userStatus.userId` keeps identity unknown.
...
`--all` attempts Codex, Claude, Grok, and Devin default contexts, plus Cursor on Linux and macOS, preserving separate observations and failures in a mixed report.
