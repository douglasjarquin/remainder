...
The v0.2.3 release adds the opt-in macOS Keychain fallback for Claude credentials; the Claude file route is natively verified.
...
The v0.4.0 release adds provider `devin` with profile `default`, also included in the fixed `--all` set.
The Linux Cursor source implementation has controlled fixtures but remains outside native release certification.
