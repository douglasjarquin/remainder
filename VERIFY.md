...
Controlled refresh uses a compiled test helper with synthetic auth and loopback TLS, independently counts one request per Codex or Grok sample and two per Claude sample, and separates helper-process timing from summed request timing to response headers.
...
Codex, Claude, and Grok integration tests use synthetic temporary auth files and loopback `httptest` servers only.
...
This policy covers Codex, Claude, and Grok collection and cache behavior with synthetic local sources and temporary cache roots.
