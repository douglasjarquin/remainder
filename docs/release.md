...
The archive includes the linked Claude, Grok, Cursor, and Devin source documents, even where a native route remains unverified.
...
The [release readiness record](release-readiness.md) separates historical Codex and Cursor evidence from the Grok patch, the v0.2.2 build-target addition, the v0.2.3 Claude Keychain fallback, the v0.3.0 TOON output, and the v0.4.0 Devin provider.
...
Version v0.4.0 adds the Devin CLI provider (PR 49, merge `ca5ac10`): `--provider devin --profile default` and a fifth entry in the fixed `--all` set, plus a nil-guard fix so an unset provider adapter reports unavailable instead of dereferencing a nil adapter.
On a cache miss, the Devin route reads the Devin CLI's own credential file — `DEVIN_CREDENTIALS`, then `$XDG_DATA_HOME/devin/credentials.toml`, then `$HOME/.local/share/devin/credentials.toml` — and makes one bounded Connect JSON `GetUserStatus` request against the credential's `api_server_url`, the same read-only route the installed CLI uses for `devin auth status`.
Daily and weekly quota percentages normalize into account windows with reset and duration; reported ACU and prompt/flow/flex credit balances map to their own windows, and the source's `-1` sentinel is preserved as unlimited rather than a number.
An absent or malformed credential file reports unavailable; there is no login, CLI invocation, token refresh, or fallback.
v0.4.0 keeps the `bin/remainder` archive layout introduced at v0.3.0: installers that extract the archive, such as mise's `github:` backend, must keep using `bin_path = "bin"` with `strip_components = 1` or they treat `docs/` and `skills/` as installed binaries.
The curl installer and Homebrew formula from the still-open PR 50 (`scripts/install.sh`, `homebrew/remainder.rb`) are not part of this release and are not an approved install path for v0.4.0; any installer that lands later must handle the `bin/` layout from the start.
Version v0.4.0 ships a `darwin_arm64` archive only; `linux_amd64` stays at v0.2.2 and `linux_arm64` at v0.2.1, and neither contains the Devin provider.
...
The Claude macOS routes are now both natively verified and the Devin macOS file route passed its 2026-09-21 canary; the Linux Cursor native route remains unverified.
...
```sh
scripts/package-release.sh v0.4.0 dist
```
...
```sh
asset=remainder_v0.4.0_darwin_arm64.tar.gz
release=https://github.com/douglasjarquin/remainder/releases/download/v0.4.0
...
Keep versions in separate directories, for example `~/.local/opt/remainder/v0.4.0/bin/remainder`, and point a user-owned symlink or explicit command path at the selected version.
...
The v0.4.0 release's `mise.toml` pins version `0.4.0` for the `macos-arm64` platform only (its `linux_amd64`/`linux_arm64` archives were not rebuilt at this version); Linux x86_64 hosts should keep using the v0.2.2 `mise.toml`, and Linux ARM64 hosts the v0.2.1 `mise.toml`.
...
```toml
[tools]
"github:douglasjarquin/remainder" = { version = "0.4.0", strip_components = 1, bin_path = "bin" }
```
...
```sh
curl --fail --location --output mise.toml https://github.com/douglasjarquin/remainder/releases/download/v0.4.0/mise.toml &&
