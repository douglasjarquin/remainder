...
The [release readiness record](release-readiness.md) separates historical Codex and Cursor evidence from the Grok patch.
...
```sh
scripts/package-release.sh v0.2.1 dist
```
...
After the `v0.2.1` assets and verification record are published, download the macOS ARM64 archive and checksum file into a new temporary directory.
...
On Linux ARM64, select `remainder_v0.2.1_linux_arm64.tar.gz` and use `sha256sum -c "$asset.sha256"` for the checksum command.
...
Keep versions in separate directories, for example `~/.local/opt/remainder/v0.2.1/remainder`, and point a user-owned symlink or explicit command path at the selected version.
Switching that pointer does not terminate an in-flight process.
Keep the existing v0.2.0 executable for rollback and retain each consumer’s existing collector until its migration is separately approved.
The v0.1.0 and v0.2.0 tags, assets, and checksum files remain immutable.

## Approved mise configuration

Use mise's explicit `github:douglasjarquin/remainder` backend.
The release's `mise.toml` pins version `0.2.1`, the exact platform archive names, and each archive's SHA-256 digest.
...
```sh
curl --fail --location --output mise.toml https://github.com/douglasjarquin/remainder/releases/download/v0.2.1/mise.toml &&
