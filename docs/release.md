...
Run the extracted binary in place first: `bin/remainder` for releases after v0.2.3, or `remainder` at the archive root for v0.2.3 and earlier (see the archive-layout note above).
...
Starting with the first release built after v0.2.3, the executable lives at `bin/remainder` inside that stripped directory, so the `[tools]` entry also needs mise's `bin_path` option; setting `bin_path` disables mise's automatic outer-directory stripping, so pair it with an explicit `strip_components = 1`:
