...
| Provider source feasibility | Selected native Codex, Claude, and macOS Cursor routes observed | [Provider source matrix](../provider-sources.md); Codex and Claude verified binding, Cursor Keychain canary; other native provider gates remain open |
| Provider collection | Codex native route, Claude native route, and macOS Cursor CLI Keychain canary verified; Grok and Linux Cursor implementations verified with controlled fixtures only | Bounded selected auth read, current native usage endpoint, normalization, and controlled HTTP/TLS tests in issue #5 |
...
| Devin file collection | Implemented; native canary verified 2026-09-21 on macOS | [Devin source, selectors, and fixture coverage](devin.md) |
...
| Published archive installation | Implemented | `scripts/install.sh` detects the host platform, resolves the newest release carrying its archive, verifies `SHA256SUMS`, and installs the binary; `homebrew/remainder.rb` is the Homebrew formula for the same archives |
