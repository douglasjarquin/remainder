...
| Standalone release packaging | Candidate tooling implemented; publication blocked | `scripts/package-release.test.sh` builds and validates the native archive, checksum, strict file allowlist, clean runtime path, Cobra help/error behavior, source metadata, and pre-publication blocker manifest |
| Codex source release readiness | Ready for independent final-SHA verification; downloaded artifact pending issue #14 | [Release readiness record and reusable checklist](../release-readiness.md) |
