...
| Provider collection | Codex native route verified; Claude and Grok file implementations verified with controlled fixtures only | Bounded selected auth read, current native usage endpoint, normalization, and controlled HTTP/TLS tests in issue #5 |
...
| Standalone release packaging | v0.1.0 published; later provider releases unverified | `scripts/package-release.test.sh` builds and validates the native archive, checksum, strict file allowlist including the MIT license, clean runtime path, Cobra help/error behavior, source metadata, and pre-publication candidate manifest |
