...
| Devin CLI (`devin 3000.10.31` observed); not a quota-axi route | Remainder selects `DEVIN_CREDENTIALS`, then `XDG_DATA_HOME/devin/credentials.toml`, otherwise `$HOME/.local/share/devin/credentials.toml`, profile `default`, and reads `windsurf_api_key` plus the `api_server_url` host it names. An explicitly empty override is invalid; there is no discovery or fallback. | Bounded noninteractive owned regular-file access on supported Unix systems, refusing symlinks and nonregular sources. No CLI invocation, login, refresh, Keychain, or browser route. | One Connect JSON `POST` to `<api_server_url>/exa.seat_management_pb.SeatManagementService/GetUserStatus` (default `https://server.codeium.com`), the same read-only call the Devin CLI uses for `devin auth status`, with `metadata.apiKey` and fixed client fields; 15-second budget, 1 MiB response bound, redirects refused. The documented `api.devin.ai` `/v3/enterprise/*` consumption endpoints require service-user permissions and are not used. | **Selected native macOS file route observed through Remainder on 2026-09-21.** The canary returned fresh complete evidence with kind `native_file_http` and name `devin_credentials_toml`, verified `userStatus.userId` binding, daily and weekly percent windows with resets and pace, and unlimited `prompt_credits`; a same-run `--cache=only` hit reused the original `observed_at` with historical identity. One host and one account were exercised. See [Devin file collection](features/devin.md). |

## What fixtures prove

The production CLI selects the native Codex, Claude, Grok, Cursor, or Devin CLI adapter for an explicit provider and `default` profile request.
`--all` attempts Codex, Claude, Grok, and Devin default contexts, adding Cursor on Linux and macOS, with independent cache and failure handling; it performs no profile discovery.
