...
Status: source review complete on 2026-09-08; one authorized external Codex feasibility canary was observed, while the Remainder provider route remains unimplemented.
...
| --- | --- | --- | --- | --- |
| OpenAI Codex CLI using a ChatGPT subscription; executable version unpinned | OAuth access token from `$CODEX_HOME/auth.json` or `~/.codex/auth.json`; the pinned source also inspects the Pi `openai-codex` entry through the [Pi Codex helper](https://github.com/kunchenguid/quota-axi/blob/d3190237588cdf51046b27a346ff2e834855bf37/src/providers/pi-codex-credential.ts) and can fall back to the configured `codex` binary. Bind a Remainder profile to the selected `CODEX_HOME` context and the returned OpenAI account ID; do not treat an API key as a ChatGPT quota identity. | The source has no OS-specific credential path. Direct file access is noninteractive. The CLI fallback starts `codex -s read-only -a untrusted app-server`; it must not start browser login or refresh from Remainder. OpenAI documents browser sign-in for `codex login` and separately documents API-key access for usage-priced work. | Try `GET https://chatgpt.com/backend-api/wham/usage`, then `GET https://chatgpt.com/backend-api/codex/usage`, with the OAuth bearer and optional `ChatGPT-Account-Id`. Fallback uses JSON-RPC `account/read` and `account/rateLimits/read` over the read-only Codex app server. | **Authorized external feasibility canary observed; Remainder route unrun. Codex release blocked.** `quota-axi@0.1.37` returned fresh read-only Codex windows twice with `--no-credential-refresh` from the selected existing context. This does not certify Remainder, whose provider implementation remains intentionally absent. |
...
This is not a Remainder provider observation: the production CLI still injects `unavailableAdapter`, so the Remainder Codex route is `unrun` until issue5 implements the proven route.
...
The prior wording "after the provider implementation exists" conflicted with issue15's dependency order and is corrected here.
The feasibility canary runs before provider implementation and supplies the proven source prerequisite for issue5; after issue5 implements that route, the same read-only scope must be rerun through Remainder before release advertising.
This task does not implement issue5.
