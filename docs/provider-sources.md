...
| Anthropic Claude Code subscription | Remainder selects `$CLAUDE_CONFIG_DIR/.credentials.json` or `$HOME/.claude/.credentials.json`, profile `default`, without discovery or fallback. An explicitly empty override is invalid. | Bounded noninteractive regular-file access on supported Unix systems, refusing symlinks and nonregular sources. No Keychain, login, refresh, or collector subprocess. | Fixed `GET https://api.anthropic.com/api/oauth/profile`, then `GET https://api.anthropic.com/api/oauth/usage`, bearer auth and beta `oauth-2025-04-20`, shared 15-second budget, redirects refused. | **Selected native route observed through Remainder.** The 2026-09-10 canary returned fresh complete observations with kind `native_file_http` and name `claude_credentials_json`, the same verified account binding across a cold and a forced-refresh sample, account/five-hour/weekly/model windows, and distinct extra-usage paid credits. The credential file was read only on the two live samples; a same-run cache hit made zero network or credential-file calls. See [Claude file collection](features/claude.md). |
