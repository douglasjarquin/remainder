...
| xAI Grok consumer and Grok CLI; executable version unpinned | Remainder selects `GROK_AUTH_JSON`, `GROK_AUTH_PATH`, `GROK_HOME/auth.json`, otherwise `$HOME/.grok/auth.json`, profile `default`. Explicitly empty overrides and inline `GROK_AUTH` are unsupported. Local account hints do not verify identity. | Bounded noninteractive owned regular-file access on supported Unix systems, refusing symlinks and nonregular sources. No login, refresh, collector subprocess, Keychain, or browser route. | Fixed `POST https://grok.com/grok_api_v2.GrokBuildBilling/GetGrokCreditsConfig` using consumer bearer auth and an empty gRPC-web message; 15-second budget, 64 KiB response bound, redirects refused. No model-catalog or API-key fallback. | **Implemented with controlled fixtures; native canary unverified.** The approved default file contained an expired consumer session. Shared/product percentages and prepaid credits remain distinct, and account identity stays unknown. See [Grok file collection](features/grok.md). |
| Cursor CLI; executable version unpinned | Remainder selects `CURSOR_CLI_CONFIG`, then `XDG_CONFIG_HOME/cursor/auth.json`, otherwise `$HOME/.config/cursor/auth.json`, profile `default`. Explicitly empty overrides are invalid. The selected source supplies an access token without verified account identity. | Linux-only bounded noninteractive owned regular-file access, refusing symlinks and unstable reads. macOS CLI Keychain and editor SQLite routes remain unsupported. No login, refresh, helper, or alternate credential route. | Three bounded `POST` requests to `https://api2.cursor.sh/aiserver.v1.DashboardService/`: `GetCurrentPeriodUsage`, `GetPlanInfo`, and `GetSandUsageStatus`, with bearer auth and empty JSON bodies, one 15-second budget, and redirects refused. | **Implemented with controlled Linux fixtures; native canary unverified.** The current macOS environment has no authorized file-only route. Included percentages and spend amounts in cents remain distinct; identity stays unknown. See [Cursor file collection](features/cursor.md). |

## What fixtures prove

The production CLI selects the native Codex, Claude, Grok, or Linux Cursor adapter for an explicit provider and `default` profile request.
`--all` attempts Codex, Claude, and Grok default contexts, adding Cursor on Linux, with independent cache and failure handling; it performs no profile discovery.
