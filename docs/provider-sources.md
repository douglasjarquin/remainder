...
The production CLI selects the native Codex, Claude, or Grok adapter for an explicit provider and `default` profile request.
`--all` attempts those three default contexts with independent cache and failure handling; it performs no profile discovery.
